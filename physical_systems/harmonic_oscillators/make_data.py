...
if __name__ == '__main__':
    ...
    # making time samples
    t0, tN = 0, 10
    samp_ts = torch.linspace(t0, tN, args.ntimestamps*10)
    import random
    mask = random.sample(list(range(1,len(samp_ts)-1)),args.ntimestamps) # irr time
    mask.sort()
    samp_ts = samp_ts[mask]
    ...
    plt.plot(samp_ts,z[:,0,0])
    # testing data
    # making time samples
    t0, tN = 0, 20
    samp_ts = torch.linspace(t0, tN, 2000)
    z = odeint(func, z0, samp_ts)
    
    np.save('data/tz0.npy', z0.numpy())
    np.save('data/tz.npy', z.numpy())
    np.save('data/tsamp_ts.npy', samp_ts.numpy())
