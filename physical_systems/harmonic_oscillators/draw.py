...
def draw(method, noise, scale, label, color, prefix = ''):
    ...
    for i in range(3):
        try :
            ...
            pass
        times[i] = np.load(filename + str(i+1) +'/time_arr.npy')
        if (plot_type =='train'):
        ...
        best_iter[i] = np.load(filename + str(i+1) +'/best_iter.npy')
    ...
    if (args.xtick == 'iter'):
        plt.plot(iters[:args.niters], lstmnode_mean[:args.niters], color=color, label=f'{label:20}, Best :{testlossmean:.2f}$\pm${testlossstd:.2f}')
        ax.fill_between(x=iters[:args.niters], y1=lstmnode_p[:args.niters], y2=lstmnode_m[:args.niters], alpha=0.2, color=color)
    else:
        plt.plot(times, lstmnode_mean, color=color, label=f'{label:20}, Best :{testlossmean:.2f}$\pm${testlossstd:.2f}')
...
plt.yscale('log')
plt.legend(fontsize=12)
plt.title('Damped Oscillators Ex MSE', fontsize=20)
plt.xlabel('Iterations', fontsize=16)
plt.ylabel('MSE', fontsize=16)
...
plt.yscale('log')
plt.legend(fontsize=12)
plt.title('Damped Oscillators In MSE', fontsize=20)
plt.xlabel('Iterations', fontsize=16)
plt.ylabel('MSE', fontsize=16)
