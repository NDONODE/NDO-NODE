...
Nf = np.concatenate((dtm,Ndf,Nf,tm),axis = 2)
Nf = torch.Tensor(Nf)
...
bs = 64
run = wandb.init(name = f"LSTM-2nd-dt-df-f-t-8-irr-ThreeBody-traj-alone-{bs}",project="diff",reinit=True)#,resume = true)
...
torch.save(best_model.state_dict(), "LSTM-2nd-dt-f-t-8-irr-traj-alone.pth")
