...
save_fig_path = './%d/%g/%.3f/%d' % (args.npoints,args.lba,args.noise,args.experiment_no)
...
mask =  np.random.choice(1000,args.npoints,replace = False)#random.sample(list(range(0,1000-1)),100)
mask.sort()
...
t_options.update({'rtol': 1e-7})
t_options.update({'atol': 1e-7})
...
if TrainMode:
    ...
    D1net.load_state_dict(torch.load('LSTM-1st-dt-f-t-5-irr-basis-alone.pth',map_location=torch.device(device)))
    D1net = D1net.to(device)
    
    dT = torch.zeros(ts.shape,dtype=torch.float32)
    for i in range(1,aL-1):
        dT[i] = ts[i]-ts[i-1]
    ...
    dT = dT.view(aL,1).to(device)
    TT = ts.view(aL,1).to(device)
    inputs1 = torch.cat((dT,r1,TT),dim = -1).view(-1,aL,5).float()
    inputs2 = torch.cat((dT,r2,TT),dim = -1).view(-1,aL,5).float()
    inputs3 = torch.cat((dT,r3,TT),dim = -1).view(-1,aL,5).float()

    with torch.no_grad():
        outs1,_ = D1net(inputs1)
        outs2,_ = D1net(inputs2)
        outs3,_ = D1net(inputs3)
    df = torch.cat((outs1,outs2,outs3),dim = -1).squeeze() / K2
    ...
    inputs1 = torch.cat((dT,r1/Scale_factor,dr1,TT),dim = -1).view(1,aL,-1).float()
    inputs2 = torch.cat((dT,r2/Scale_factor,dr2,TT),dim = -1).view(1,aL,-1).float()
    inputs3 = torch.cat((dT,r3/Scale_factor,dr3,TT),dim = -1).view(1,aL,-1).float()

    import argparse
    config = argparse.ArgumentParser()
    config.out_dim = 3
    config.embedding_dim = 6+2
    config.hidden_dim = 64
    config.layers_num = 2
    config.dropout = 0
    D2net = LstmModel(embedding_dim=config.embedding_dim,
                ...
                layers_num=config.layers_num,
                out_dim=config.out_dim)
    D2net.load_state_dict(torch.load('LSTM-2nd-dt-f-df-t-8-irr-basis-alone.pth',map_location=torch.device(device)))
    D2net = D2net.to(device)

    with torch.no_grad():
        outp1,_ = D2net(inputs1)
        outp2,_ = D2net(inputs2)
        outp3,_ = D2net(inputs3)
    outp2 = torch.cat((outp1,outp2,outp3),dim = -1).squeeze()

    out_all_data = df.cpu() #/ K2
    ...
    for _epoch in range(num_epochs): 
        ...
        out = odeint(func, initial_condition, torch.from_numpy(np.array(t_list)) ,method = options['method'].lower())# options=options)#, time_points=t_list)
        ...
        inp = torch.cat((fs,df),dim=-1).to(device)
        out = func(ts, inp)
        label =torch.cat((df*K2,outp2),dim=-1) # diff -> ddf[mask,:]/K2)
        b = F.mse_loss(out,label)
        a = torch.sum(torch.abs(dif2)) / float(time_span.shape[0])
